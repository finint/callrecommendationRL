...
#x,y,feature_columns,behavior_feature_list=get_DIN_DIEN_data(call_data,user_embedding_list,label_sheet_list,date_everyday)
if model_name=="DIN":
    model = DIN(feature_columns, behavior_feature_list, l2_reg_dnn=reg_c,dnn_hidden_units=[10], att_hidden_size=[10])
    model.compile('adam', 'binary_crossentropy',metrics=['binary_crossentropy'])
    model.fit(x, y, verbose=2, epochs=train_epoch)
elif model_name=="DIEN":
    if tf.__version__ >= '2.0.0':
        tf.compat.v1.disable_eager_execution()
    model = DIEN(feature_columns, behavior_feature_list, l2_reg_dnn=reg_c,
        dnn_hidden_units=[30], att_hidden_units=[30], 
        gru_type="GRU")
    model.compile('adam', 'binary_crossentropy',metrics=['binary_crossentropy'])
    model.fit(x, y, verbose=2, epochs=train_epoch)
